...
class FilesWidget {
    ...
    private PadLength = 15;

    private PID: string;
    private URL: string;

    private $FolderContextMenu: JQuery;
    private $FileContextMenu: JQuery;


    private CONNECTED: boolean;

    private $SelectedEntry: JQuery;
     ...
     */
    constructor($e: JQuery, Title: string = "Project Files") {
        this.$ELEMENT = $e;
        $e.empty();
     ...
     */
    public Load(RootFiles: Array<IFileStructure>, $Parent: JQuery) {
        if (!$Parent) return;
         ...
         */
        var RecursiveAdd = (Files: IFileStructure[], Padding: number = 0, ParentPath: string = "") => {
            if (!!Files && Files.length > 0) {
                var entry: IFileStructure;
                ...
                for (var i = 0; i < Files.length; i++) {
                    ...
                    if (entry.IS_DIR == "1") {
                        ...
                        $Entry.click(
                            ...
                            // Why? Because else the Variable "MyPath" references to the last state and not the current value we want to store
                                ((Path: string) =>
                                    () => { this.HandleCollapse(Path); }
                                )(MyPath)
                            )
                            // Context Menu Handling for Folders
                            .contextmenu(
                                MyPath,
                    ...
                    else // If current entry is a file
                    {
                        ...
                        $Entry.addClass("file")
                            ...
                            // Store Hash ID of this File for AJAX loading
                            .attr("data-file-id", entry.ID)
                            // Context Menu Handling for Files
                            .contextmenu(
                                entry.ID,
                                (e: JQueryMouseEventObject) => {
                                    e.preventDefault();
    ...
    public HandleFileContextMenu(FileID: string, event: JQueryMouseEventObject) {
     ...
     */
    public HandleFolderContextMenu(FolderId: string, event: JQueryMouseEventObject) {
        // get list item as JQuery Object
     ...
     * @param event
     */
    public HandleContextMenuOut(event: JQueryMouseEventObject) {
     ...
     * @param $Children
     */
    private HideCollapsedChildren($Children: JQuery) {
     ...
     * @param FolderPath
     */
    private HandleCollapse(FolderID: string) {
    ...
    public static CharByNum(Num: number): string {
    ...
    public HandleFileOpen(File: IFileStructure) {
        if (!File) {
    ...
    private CreateNewFolder() {
        if (!this.CONNECTED || !this.$SelectedEntry) return;
    }
